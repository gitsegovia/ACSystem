...
const RolesCards = () => {
  // ** States
  const inputRole=useRef<string>()
  const [submitRole,setSubmitRole]=useState<boolean>(false)
  const [rolesArr,setRolesArr] = useState<string[]>(ROLES_ARR)
  ...
  // funcion que cambia el estado de los checks del select all
  const handlePermissionAll =(id:string)=>{
  ...
  const handleSelectAllCheckbox = () => {
    setIsIndeterminateCheckbox((prev)=> !prev)
  }


  const handleInputRole=({target}:Event)=>{
    if(target){
      
    inputRole.current=(target as HTMLInputElement).value
  ...
  useEffect(()=>{
if(submitRole) {
  setRolesArr((prev)=> [...prev,inputRole.current as string])
  setSubmitRole(false)
}
  ...
  // efecto al cambio del selectAll
  useEffect(() => {
    ...
    if (!isIndeterminateCheckbox) {
      ...
      rolesArr.forEach(row => {
       ...
       handlePermissionAll(`${id}-read`)
        handlePermissionAll(`${id}-write`)
        handlePermissionAll(`${id}-create`)
        ...
        >
          <Box sx={{ my: 4 }}>
            <FormControl fullWidth>
              <TextField label='Search Role Name' placeholder='Enter Role Name' onInput={handleInputRole} />
            </FormControl>
          </Box>
