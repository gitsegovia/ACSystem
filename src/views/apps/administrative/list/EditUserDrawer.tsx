...
const SidebarAddUser = (props: SidebarAddUserType) => {
  // ** Props
  const { open, toggle, update, infoAdmin } = props;
  ...
  const {
    ...
    defaultValues: {
      ...
      firstName: infoAdmin.firstName,
      lastName: infoAdmin.lastName,
      idnDni: infoAdmin.idnDni,
      phone: infoAdmin.phone,
      address: infoAdmin.address,
      position: infoAdmin.position,
      gender: infoAdmin.gender,
    } as UserData,
    mode: "onChange",
  ...
  const onSubmit = (dataUser: UserData) => {
    dispatch(
      addUser({
        mutate,
        query,
        data: { ...dataUser, id: infoAdmin.id },
        callback: (data, error) => {
          if (data && data === true) {
            toggle();
            if (update) {
              update({ ...infoAdmin, ...dataUser });
            }
          } else {
            toast.error(error);
          }
  ...
  return (
    <Drawer open={open} anchor="right" variant="temporary" onClose={handleClose} ModalProps={{ keepMounted: true }} sx={{ "& .MuiDrawer-paper": { width: { xs: 300, sm: 400 } } }}>
      <Header>
        <Typography variant="h6">Editar personal</Typography>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <FormControl fullWidth sx={{ mb: 6 }}>
            <Controller
              ...
              control={control}
              rules={{ required: true }}
              render={({ field: { value, onChange } }) => <TextField type="Correo" value={value} label="Email" onChange={onChange} placeholder="johndoe@email.com" error={Boolean(errors.email)} />}
            />
