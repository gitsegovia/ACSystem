...
import UserSuspendDialog from "src/views/apps/administrative/view/UserSuspendDialog";
...
const UserViewLeft = ({ infoAdministrative }: Props) => {
  // ** States
  const [openEdit, setOpenEdit] = useState<boolean>(false);
  const [suspendDialogOpen, setSuspendDialogOpen] = useState<boolean>(false);
  const [info, setInfo] = useState(infoAdministrative);
        ...
        <Grid item xs={12}>
          <Card sx={{ boxShadow: "none", border: (theme) => `2px solid ${theme.palette.primary.main}` }}>
            <CardContent sx={{ display: "flex", flexWrap: "wrap", pb: "0 !important", justifyContent: "space-between" }}>
              <ShowCodeQr qrCode={`https://qr.guarico.gob.ve/${info.idnDni}`} />
            </CardContent>
          </Card>
        </Grid>
      </Grid>
